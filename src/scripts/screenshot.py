...
# jcsv = CSVReader(FileName=['/media/nimda/ボリューム/MatsuoLab/life-hts/scripts/periodic3d/j.csv'])
# create a new 'CSV Reader'
...
renderView1.ViewSize = [3294, 1800]
...
renderView1.CameraPosition = [-0.000329239147997299, -0.001784881664798335, 0.0014232148867900877]
renderView1.CameraFocalPoint = [2.734304871410225e-08, -2.691813278943296e-07, 0.00019576232671170152]
renderView1.CameraViewUp = [-0.0591595638694988, 0.5730897283671907, 0.8173544575291618]
renderView1.CameraParallelScale = 0.0005702709201287946

import sys

process_id = float(sys.argv[1]) - 1
num_process = 8
num_timestep = 122.

# Properties modified on animationScene1
animationScene1.AnimationTime = process_id
while int(animationScene1.AnimationTime) <= num_timestep:
    # save screenshot
    SaveScreenshot('/media/nimda/ボリューム/MatsuoLab/life-hts/scripts/ferromulticore/animj/animj'+str(int(animationScene1.AnimationTime))+'.png', renderView1, ImageResolution=[3294, 1800])
    animationScene1.AnimationTime += num_process

# save animation
# SaveAnimation('/media/nimda/ボリューム/MatsuoLab/life-hts/scripts/animj.png', renderView1, ImageResolution=[1021, 1801],
#     FrameWindow=[0, 122])
