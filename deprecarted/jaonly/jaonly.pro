...
Formulation {
  { Name MagDynH; Type FemEquation;
    ...
    Equation {
      ...
      Galerkin { [ -(SetVariable[$RelaxFac*b_Jiles[{b}[1],{h}[1],{h}]{List[hyst_Fe]}+(1-$RelaxFac)*{b},QuadraturePointIndex[]]{$bjiles}) , {b} ];
        In MagnAnhyDomain ; Jacobian Vol ; Integration Int; }
      ...
      Galerkin { [ -{b}[1] / $DTime , {h} ];
        In MagnAnhyDomain; Integration Int; Jacobian Vol;  }
      Galerkin { [ 1/$RelaxFac* (SetVariable[dbdh_Jiles[{h},{b},{h}-{h}[1]]{List[hyst_Fe]},QuadraturePointIndex[]]{$dhdbjiles}) * Dof{h} / $DTime , {h}];
        In MagnAnhyDomain; Integration Int; Jacobian Vol;  }
      Galerkin { [ -1/$RelaxFac* (GetVariable[QuadraturePointIndex[]]{$dhdbjiles}) * {h} / $DTime , {h}];
        In MagnAnhyDomain; Integration Int; Jacobian Vol;  }
